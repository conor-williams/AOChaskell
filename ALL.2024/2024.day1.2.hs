...
main = do
    ...
    case args of
        [filename] -> do
            ...
            let zz = loopleft le1 ri1 0
            print zz

loopleft :: [Int] -> [Int] -> Int -> Int
loopleft [] ri1 tot = tot
loopleft (el:le1) ri1 tot = do
    let xx = loopOccur ri1 el
    let tot2 = tot + (xx * el)
    let qq = loopleft le1 ri1 tot2
    qq
     
loopOccur :: [Int] -> Int -> Int
loopOccur xs n = length [x | x <- xs, x == n] 
