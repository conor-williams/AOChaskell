...
loopThrough (str:vv) list1 cnt pos = do
    ...
    let thi = extractInt (list1 !! (pos+1))
    let thi2 = convertMaybeToInt thi
    let fou = extractInt (list1 !! (pos+2))
    let fou2 = convertMaybeToInt fou

    if (pos+2) == (length list1) then do
        cnt
    else do 
        let gr1 = amt2 + sec2 + thi2
        let gr2 = sec2 + thi2 + fou2
	if gr2 > gr1 then do
